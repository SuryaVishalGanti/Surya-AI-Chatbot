...
# Function to set background image for the main app
...
def main():
    ...
    font_size = st.slider("Select Font Size for Chat History", min_value=12, max_value=30, value=18)  # Dynamic font size
    ...
    if option == "Surya's AI Chatbot":
        ...
        for message in st.session_state.conversation_history:
            if message.startswith("You:"):
                st.markdown(f"**<div style='color:#3498db;'>{message}</div>**", unsafe_allow_html=True)  # User message in blue
            else:
                st.markdown(f"<div style='color:#e74c3c;'>{message}</div>", unsafe_allow_html=True)  # AI message in red
